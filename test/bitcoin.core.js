...
describe('fabcoin-core', function () {
  // base58EncodeDecode
  ...
  describe('address.toBase58Check', function () {
    ...
    base58KeysValid.forEach(function (f) {
      ...
      const network = params.isTestnet ? fabcoin.networks.testnet : fabcoin.networks.fabcoin
      const version = network[typeMap[params.addrType]]

      it('can export ' + expected, function () {
        assert.strictEqual(fabcoin.address.toBase58Check(hash, version), expected)
      })
    })
  })
  ...
  describe('address.fromBase58Check', function () {
    const allowedNetworks = [
      fabcoin.networks.fabcoin.pubkeyhash,
      fabcoin.networks.fabcoin.scripthash,
      fabcoin.networks.testnet.pubkeyhash,
      fabcoin.networks.testnet.scripthash
    ]
    ...
    base58KeysInvalid.forEach(function (f) {
      ...
      it('throws on ' + string, function () {
        assert.throws(function () {
          const address = fabcoin.address.fromBase58Check(string)
  ...
  describe('ECPair', function () {
    base58KeysValid.forEach(function (f) {
      ...
      const network = params.isTestnet ? fabcoin.networks.testnet : fabcoin.networks.fabcoin
      const keyPair = fabcoin.ECPair.fromWIF(string, network)
  ...
  describe('ECPair.fromWIF', function () {
    ...
    base58KeysInvalid.forEach(function (f) {
      ...
      it('throws on ' + string, function () {
        assert.throws(function () {
          fabcoin.ECPair.fromWIF(string, allowedNetworks)
  ...
  describe('Block.fromHex', function () {
    blocksValid.forEach(function (f) {
      it('can parse ' + f.id, function () {
        const block = fabcoin.Block.fromHex(f.hex)
  ...
  describe('Transaction.fromHex', function () {
    txValid.forEach(function (f) {
      ...
      it('can decode ' + fhex, function () {
        const transaction = fabcoin.Transaction.fromHex(fhex)
  ...
  describe('Transaction', function () {
    sigHash.forEach(function (f) {
      ...
      const hashTypes = []
      if ((hashType & 0x1f) === fabcoin.Transaction.SIGHASH_NONE) hashTypes.push('SIGHASH_NONE')
      else if ((hashType & 0x1f) === fabcoin.Transaction.SIGHASH_SINGLE) hashTypes.push('SIGHASH_SINGLE')
      else hashTypes.push('SIGHASH_ALL')
      if (hashType & fabcoin.Transaction.SIGHASH_ANYONECANPAY) hashTypes.push('SIGHASH_ANYONECANPAY')

      const hashTypeName = hashTypes.join(' | ')

      it('should hash ' + txHex.slice(0, 40) + '... (' + hashTypeName + ')', function () {
        const transaction = fabcoin.Transaction.fromHex(txHex)
        assert.strictEqual(transaction.toHex(), txHex)

        const script = Buffer.from(scriptHex, 'hex')
        const scriptChunks = fabcoin.script.decompile(script)
        assert.strictEqual(fabcoin.script.compile(scriptChunks).toString('hex'), scriptHex)
  ...
  describe('script.signature.decode', function () {
    sigCanonical.forEach(function (hex) {
      ...
      it('can parse ' + hex, function () {
        const parsed = fabcoin.script.signature.decode(buffer)
        const actual = fabcoin.script.signature.encode(parsed.signature, parsed.hashType)
    ...
    sigNoncanonical.forEach(function (hex, i) {
      ...
      it('throws on ' + description, function () {
        assert.throws(function () {
          fabcoin.script.signature.decode(buffer)
