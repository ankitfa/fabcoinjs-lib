...
describe('fabcoinjs-lib (transactions)', function () {
  it('can create a 1-to-1 Transaction', function () {
    const alice = fabcoin.ECPair.fromWIF('L1uyy5qTuGrVXrmrsvHWHgVzW9kKdrp27wBC7Vs6nZDTF2BRUVwy')
    const txb = new fabcoin.TransactionBuilder()
    ...
    // prepare for broadcast to the Fabcoin network, see "can broadcast a Transaction" below
  ...
  it('can create a 2-to-2 Transaction', function () {
    const alice = fabcoin.ECPair.fromWIF('L1Knwj9W3qK3qMKdTvmg3VfzUs3ij2LETTFhxza9LfD5dngnoLG1')
    const bob = fabcoin.ECPair.fromWIF('KwcN2pT3wnRAurhy7qMczzbkpY5nXMW2ubh696UBc1bcwctTx26z')
  ...
  it('can create (and broadcast via 3PBP) a typical Transaction', function (done) {
    const alice1 = fabcoin.ECPair.makeRandom({ network: regtest })
    const alice2 = fabcoin.ECPair.makeRandom({ network: regtest })
    const aliceChange = fabcoin.ECPair.makeRandom({ network: regtest, rng: rng })

    const alice1pkh = fabcoin.payments.p2pkh({ pubkey: alice1.publicKey, network: regtest })
    const alice2pkh = fabcoin.payments.p2pkh({ pubkey: alice2.publicKey, network: regtest })
    const aliceCpkh = fabcoin.payments.p2pkh({ pubkey: aliceChange.publicKey, network: regtest })
    ...
    regtestUtils.faucet(alice1pkh.address, 5e4, function (err, unspent0) {
      ...
      regtestUtils.faucet(alice2pkh.address, 7e4, function (err, unspent1) {
        ...
        // to build and broadcast to the actual Fabcoin network, see https://github.com/fabcoinjs/fabcoinjs-lib/issues/839
      })
    })
  })

  it('can create (and broadcast via 3PBP) a Transaction with an OP_RETURN output', function (done) {
    const keyPair = fabcoin.ECPair.makeRandom({ network: regtest })
    const p2pkh = fabcoin.payments.p2pkh({ pubkey: keyPair.publicKey, network: regtest })

    regtestUtils.faucet(p2pkh.address, 2e5, function (err, unspent) {
      if (err) return done(err)

      const txb = new fabcoin.TransactionBuilder(regtest)
      const data = Buffer.from('fabcoinjs-lib', 'utf8')
      const embed = fabcoin.payments.embed({ data: [data] })
  ...
  it('can create (and broadcast via 3PBP) a Transaction, w/ a P2SH(P2MS(2 of 4)) (multisig) input', function (done) {
    const keyPairs = [
      fabcoin.ECPair.makeRandom({ network: regtest }),
      fabcoin.ECPair.makeRandom({ network: regtest }),
      fabcoin.ECPair.makeRandom({ network: regtest }),
      fabcoin.ECPair.makeRandom({ network: regtest })
    ]
    const pubkeys = keyPairs.map(x => x.publicKey)
    const p2ms = fabcoin.payments.p2ms({ m: 2, pubkeys: pubkeys, network: regtest })
    const p2sh = fabcoin.payments.p2sh({ redeem: p2ms, network: regtest })
    ...
    regtestUtils.faucet(p2sh.address, 2e4, function (err, unspent) {
      ...
      // build and broadcast to the Fabcoin RegTest network
  ...
  it('can create (and broadcast via 3PBP) a Transaction, w/ a P2SH(P2WPKH) input', function (done) {
    const keyPair = fabcoin.ECPair.makeRandom({ network: regtest })
    const p2wpkh = fabcoin.payments.p2wpkh({ pubkey: keyPair.publicKey, network: regtest })
    const p2sh = fabcoin.payments.p2sh({ redeem: p2wpkh, network: regtest })
  ...
  it('can create (and broadcast via 3PBP) a Transaction, w/ a P2WPKH input', function (done) {
    ...
    regtestUtils.faucetComplex(p2wpkh.address, 5e4, function (err, unspent) {
      ...
      // build and broadcast (the P2WPKH transaction) to the Fabcoin RegTest network
      regtestUtils.broadcast(tx.toHex(), function (err) {
        if (err) return done(err)
  ...
  it('can create (and broadcast via 3PBP) a Transaction, w/ a P2WSH(P2PK) input', function (done) {
    const keyPair = fabcoin.ECPair.makeRandom({ network: regtest })
    const p2pk = fabcoin.payments.p2pk({ pubkey: keyPair.publicKey, network: regtest })
    const p2wsh = fabcoin.payments.p2wsh({ redeem: p2pk, network: regtest })
    ...
    regtestUtils.faucetComplex(p2wsh.address, 5e4, function (err, unspent) {
      ...
      // build and broadcast (the P2WSH transaction) to the Fabcoin RegTest network
      regtestUtils.broadcast(tx.toHex(), function (err) {
        if (err) return done(err)
  ...
  it('can create (and broadcast via 3PBP) a Transaction, w/ a P2SH(P2WSH(P2MS(3 of 4))) (SegWit multisig) input', function (done) {
    ...
    const p2ms = fabcoin.payments.p2ms({ m: 3, pubkeys, network: regtest })
    const p2wsh = fabcoin.payments.p2wsh({ redeem: p2ms, network: regtest })
    const p2sh = fabcoin.payments.p2sh({ redeem: p2wsh, network: regtest })
  ...
  it('can verify Transaction signatures', function () {
    ...
    ].map(function (q) { return fabcoin.ECPair.fromPublicKey(Buffer.from(q, 'hex')) })

    const tx = fabcoin.Transaction.fromHex(txHex)

    tx.ins.forEach(function (input, i) {
      const keyPair = keyPairs[i]
      const p2pkh = fabcoin.payments.p2pkh({
        pubkey: keyPair.publicKey,
        input: input.script
      })

      const ss = fabcoin.script.signature.decode(p2pkh.signature)
