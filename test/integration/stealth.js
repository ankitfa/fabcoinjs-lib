...
function stealthSend (e, Q) {
  ...
  const c = fabcoin.crypto.sha256(eQ)
  const Qc = ecc.pointAddScalar(Q, c)
  const vG = fabcoin.ECPair.fromPublicKey(Qc)
...
function stealthReceive (d, eG) {
  ...
  const v = fabcoin.ECPair.fromPrivateKey(dc)
...
function stealthRecoverLeaked (v, e, Q) {
  ...
  const d = fabcoin.ECPair.fromPrivateKey(vc)
...
function stealthDualSend (e, R, Q) {
  ...
  const vG = fabcoin.ECPair.fromPublicKey(Rc)
...
function stealthDualReceive (d, r, eG) {
  ...
  const v = fabcoin.ECPair.fromPrivateKey(rc)
...
describe('fabcoinjs-lib (crypto)', function () {
  it('can generate a single-key stealth address', function () {
    ...
    const recipient = fabcoin.ECPair.fromWIF('5KYZdUEo39z3FPrtuX2QbbwGnNP5zTd7yyr2SC1j299sBCnWjss') // private to recipient
    const nonce = fabcoin.ECPair.fromWIF('KxVqB96pxbw1pokzQrZkQbLfVBjjHFfp2mFfEp8wuEyGenLFJhM9') // private to sender
  ...
  it('can generate a single-key stealth address (randomly)', function () {
    const recipient = fabcoin.ECPair.makeRandom() // private to recipient
    const nonce = fabcoin.ECPair.makeRandom() // private to sender
  ...
  it('can generate a dual-key stealth address', function () {
    // XXX: should be randomly generated, see next test for example
    const recipient = fabcoin.ECPair.fromWIF('5KYZdUEo39z3FPrtuX2QbbwGnNP5zTd7yyr2SC1j299sBCnWjss') // private to recipient
    const scan = fabcoin.ECPair.fromWIF('L5DkCk3xLLoGKncqKsWQTdaPSR4V8gzc14WVghysQGkdryRudjBM') // private to scanner/recipient
    const nonce = fabcoin.ECPair.fromWIF('KxVqB96pxbw1pokzQrZkQbLfVBjjHFfp2mFfEp8wuEyGenLFJhM9') // private to sender
  ...
  it('can generate a dual-key stealth address (randomly)', function () {
    const recipient = fabcoin.ECPair.makeRandom() // private to recipient
    const scan = fabcoin.ECPair.makeRandom() // private to scanner/recipient
    const nonce = fabcoin.ECPair.makeRandom() // private to sender
