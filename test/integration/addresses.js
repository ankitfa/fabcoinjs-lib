...
describe('fabcoinjs-lib (addresses)', function () {
  it('can generate a random address', function () {
    const keyPair = fabcoin.ECPair.makeRandom({ rng: rng })
    const { address } = fabcoin.payments.p2pkh({ pubkey: keyPair.publicKey })

    assert.strictEqual(address, '1F5VhMHukdnUES9kfXqzPzMeF1GPHKiF64')
  })

  it('can generate an address from a SHA256 hash', function () {
    const hash = fabcoin.crypto.sha256(Buffer.from('correct horse battery staple'))

    const keyPair = fabcoin.ECPair.fromPrivateKey(hash)
    const { address } = fabcoin.payments.p2pkh({ pubkey: keyPair.publicKey })
  ...
  it('can import an address via WIF', function () {
    const keyPair = fabcoin.ECPair.fromWIF('Kxr9tQED9H44gCmp6HAdmemAzU3n84H3dGkuWTKvE23JgHMW8gct')
    const { address } = fabcoin.payments.p2pkh({ pubkey: keyPair.publicKey })
  ...
  it('can generate a P2SH, pay-to-multisig (2-of-3) address', function () {
    ...
    const { address } = fabcoin.payments.p2sh({
      redeem: fabcoin.payments.p2ms({ m: 2, pubkeys })
    })
  ...
  it('can generate a SegWit address', function () {
    const keyPair = fabcoin.ECPair.fromWIF('Kxr9tQED9H44gCmp6HAdmemAzU3n84H3dGkuWTKvE23JgHMW8gct')
    const { address } = fabcoin.payments.p2wpkh({ pubkey: keyPair.publicKey })
  ...
  it('can generate a SegWit address (via P2SH)', function () {
    const keyPair = fabcoin.ECPair.fromWIF('Kxr9tQED9H44gCmp6HAdmemAzU3n84H3dGkuWTKvE23JgHMW8gct')
    const { address } = fabcoin.payments.p2sh({
      redeem: fabcoin.payments.p2wpkh({ pubkey: keyPair.publicKey })
    })
  ...
  it('can generate a P2WSH (SegWit), pay-to-multisig (3-of-4) address', function () {
    ...
    const { address } = fabcoin.payments.p2wsh({
      redeem: fabcoin.payments.p2ms({ m: 3, pubkeys })
    })
  ...
  it('can support the retrieval of transactions for an address (via 3PBP)', function (done) {
    const keyPair = fabcoin.ECPair.makeRandom()
    const { address } = fabcoin.payments.p2pkh({ pubkey: keyPair.publicKey })
  ...
  it('can generate a Testnet address', function () {
    const testnet = fabcoin.networks.testnet
    const keyPair = fabcoin.ECPair.makeRandom({ network: testnet, rng: rng })
    const wif = keyPair.toWIF()
    const { address } = fabcoin.payments.p2pkh({ pubkey: keyPair.publicKey, network: testnet })
  ...
  it('can generate a Litecoin address', function () {
    const keyPair = fabcoin.ECPair.makeRandom({ network: LITECOIN, rng: rng })
    const wif = keyPair.toWIF()
    const { address } = fabcoin.payments.p2pkh({ pubkey: keyPair.publicKey, network: LITECOIN })
